...
//arreglo que posee todos los pedidos (a través de objetos) cargados por el usuario en la tienda (separados por equipo y tipo de producto)
let arrayPedidos = []
    //variable que funciona como un contador, y permite asignar un id a cada pedido nuevo que se carga en el array de pedidos
let idPedidos = 0
    //variables
    // let llaverosCASLA = 0
    // let imanesCASLA = 0
    ...
    //constantes en donde se setean los valores unitarios de los dos tipos de productos que vende la tienda (imanes y llaveros). Esto es con el fin de que cualquier desarrollador pueda modificar los valores de los tipos de productos sin tener que pensar tanto en la lógica del algoritmo.
const VALOR_UNIDAD_LLAVERO = 170
const VALOR_UNIDAD_IMAN = 200
    //constantes en donde se setean los porcentajes de aumentos de acuerdo a la cantidad de cuotas que elija para pagar el usuario.
const PORCENTAJE_AUMENTO_3CUOTAS = 5
const PORCENTAJE_AUMENTO_6CUOTAS = 10
const PORCENTAJE_AUMENTO_12CUOTAS = 15

//constantes que poseen la ruta en donde se encuentran las imágenes de los productos que vende la tienda (se utilizan al momento de agregar las 'cards' con los pedidos en la tienda a través del DOM)
...
//declaración de variables vinculadas con DOM (conexión con elementos HTML)
//elementos HTML vinculados con el form de pedidos
let selectorEquipo
...
//2.d - Declaración de clase 'Precios': permite generar un objeto similar a un "catálogo", que en sus propiedades contiene a los valores unitarios de los productos que se venden en la tienda + el porcentaje de recargo que establece el sistema de acuerdo a las cuotas que elija el cliente para pagar
class Precios {
...
//devuelve "true" si el usuario tiene algún pedido realizado, y sino "false"
...
//función que recibe como parámetro el arreglo de pedidos actual que posee la tienda, y devuelve un objeto de clase "Carrito" con toda la información de los pedidos pasados por parámetro
...
function inicializarDOM() {
    ...
    selectorCuotas.onchange = mostrarRecargoCuotas
    ...
    formularioDatosAdicionales.onsubmit = (event) => confirmarCompra(event)
    botonResetearEcommerce = document.getElementById("resetearEcommerce")
    botonResetearEcommerce.onclick = resetearEcommerce
    ...
    contenedorPrincipalCorreos = document.getElementById("contenedorPrincipalCorreos")
    cajaCorreos = document.getElementById("cajaCorreos")
    botonconsultarHorariosDeAtencion = document.getElementById("consultarHorariosDeAtencion")
    botonconsultarHorariosDeAtencion.onclick = mostrarHorariosDeAtencion
    labelRecargoCuotasSeleccionadas = document.getElementById("recargoCuotasSeleccionadas")
}


function mostrarRecargoCuotas() {
    let cuotasSeleccionadas = selectorCuotas.value
    let porcentajeAMostrar
    switch (cuotasSeleccionadas) {
        case "1":
            porcentajeAMostrar = 0
            break
        case "3":
            porcentajeAMostrar = PORCENTAJE_AUMENTO_3CUOTAS
            break
        case "6":
            porcentajeAMostrar = PORCENTAJE_AUMENTO_6CUOTAS
            break
        case "12":
            porcentajeAMostrar = PORCENTAJE_AUMENTO_12CUOTAS
            break
        default:
            break
    }
    let cadenaAMostrar = "Recargo por cuotas seleccionadas: " + porcentajeAMostrar + "%"
    labelRecargoCuotasSeleccionadas.innerText = cadenaAMostrar
}

//funcion que limpia los campos de todos los forms
...
//función a través de la cual se loguea un usuario en la tienda
...
//función que chequea, dentro del array de pedidos, si ya existe un pedido que tenga a un equipo y a un tipo de producto como los que se especifican en los parámmetros. en caso de existir, devuelve el index en el arreglo, y sino, devuelve "-1"
...
//función que agrega un pedido al array de pedidos, teniendo en cuenta los parámetros especificados por el usuario en el form
...
function pintarNuevoPedido(pedidoAPintar, esNuevoPedido) {
    ...
    if (esNuevoPedido) {
        nuevaCard.classList.add("wow")
        nuevaCard.classList.add("fadeIn")
    }
    nuevaCard.innerHTML = `
...
function resetearEcommerce(mostrarToast = true) {
    ...
    //limpio los forms
    limpiarTodosLosForms()
    labelRecargoCuotasSeleccionadas.innerText = "Recargo por cuotas seleccionadas: 0%"
        //muestro toast de confirmación si el usuario lo desea
    if (mostrarToast) {
        mostrarToastConfirmacion("Tienda reiniciada")
    }
}

async function mostrarHorariosDeAtencion() {
    try {
        let respuestaServidor = await fetch("https://63442e20dcae733e8fd9768f.mockapi.io/horariosatencion")
        let arrayRespuestaServidor = await respuestaServidor.json()
        let objetoHorarios = arrayRespuestaServidor[0]
        let nroWhatsApp = objetoHorarios.whatsapp
        let correoElectronico = objetoHorarios.correo
        let horariosSemanales = objetoHorarios.horarios
        let mensajeAMostrar = `
        <strong>Número de WhatsApp:</strong> ${nroWhatsApp} <br>
        <strong>Correo electrónico:</strong> ${correoElectronico} <br>
        <strong>Horarios:</strong><br>
        Lunes: ${horariosSemanales.lunes} <br>
        Martes: ${horariosSemanales.martes} <br>
        Miércoles: ${horariosSemanales.miercoles} <br>
        Jueves: ${horariosSemanales.jueves} <br>
        Viernes: ${horariosSemanales.viernes} <br>
        Sabado: ${horariosSemanales.sabado} <br>
        Domingo:  <strong>${horariosSemanales.domingo}</strong>`
        mostrarSweetAlertInformativo("Contacto y horarios", mensajeAMostrar)
    } catch (error) {
        console.log(error)
    }
}

async function pintarCorreosDOM() {
    try {
        let respuestaServidor = await fetch("https://63442e20dcae733e8fd9768f.mockapi.io/empresas")
        let arrayEmpresas = await respuestaServidor.json()
        for (let objetoEmpresa of arrayEmpresas) {
            let nombreEmpresa = objetoEmpresa.nombre
            let demoraEmpresa = objetoEmpresa.demora
            let rutaLogoEmpresa = objetoEmpresa.rutaLogo
            let rutaWebEmpresa = objetoEmpresa.web
            let altLogoEmpresa = "Logo de la empresa " + nombreEmpresa
            let correoAPintar = document.createElement('div')
            correoAPintar.classList.add("cajaCorreos__item")
            correoAPintar.innerHTML = `
        <div class="card personalizacionCard2" style="width: 15rem;">
            <div class="card-body">
                <img class="card-img-top" src="${rutaLogoEmpresa}" alt="${altLogoEmpresa}">
                <h5 class="card-title tituloCards">${nombreEmpresa}</h5>
                <p class="card-text">Demora en el servicio: <strong>${demoraEmpresa} horas</strong></p>
                <a href="${rutaWebEmpresa}" target="_blank" class="btn btn-secondary">Ir a la web</a>
            </div>
        </div>`
            cajaCorreos.append(correoAPintar)
        }
        contenedorPrincipalCorreos.hidden = false
    } catch (error) {
        console.log(error)
    }
}
...
function refrescarValoresPagina() {
    //pinto los correos disponibles para realizar envios en función de lo que indica el servidor de 'mockAPI'
    pintarCorreosDOM()
...
function pintarTodosLosPedidos(hacerAnimacion) {
    pizarraPedidos.innerHTML = ""
    for (const pedido of arrayPedidos) {
        pintarNuevoPedido(pedido, hacerAnimacion)
    }
}
...
function mostrarSweetAlertInformativo(titulo, descripcion) {
    Swal.fire({
        icon: 'info',
        title: titulo,
        html: descripcion,
...
//funcion que le permite al usuario confirmar la eliminación de un producto particular
...
//funcion que le permite al usuario confirmar su salida de la sesión
function confirmarFinSesionUsuario() {
    Swal.fire({
        title: 'Desea cerrar sesión?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#328B20',
        cancelButtonColor: '#d33',
...
//funcion que le permite al usuario confirmar la compra de los pedidos encargados.
    ...
    if (inputDireccion.value == "") {
        ...
        if (usuarioCompro()) {
            ...
            let mensajeResumenPedido =
                `<h5><strong>Resumen de productos por equipo</strong></h5>
        ...
        `
            let mensajeValoresCarrito =
                `<h5><strong>Valores del carrito</strong></h5>
        ...
        `
            let mensajeSubTotal = `<h5><strong>Subtotal:</strong> $${carritoDeCompra.subTotal}</h5>`
            let cantidadCuotasElegidas = `<h5><strong>Cantidad de cuotas:</strong> ${selectorCuotas.value}</h5>`
            let recargoCuotasElegidas
            switch (selectorCuotas.value) {
                case "1":
            ...
            recargoCuotasElegidas = `<h5><strong>Recargo por las cuotas elegidas:</strong> ${recargoCuotasElegidas}</h5>`
            let mensajeTotalAAbonar = `<h5><strong>Total a abonar:</strong> $${carritoDeCompra.devolverValorFinal(catalogoPrecios)}</h5>`
            let mensajeTotalAAbonarPorCuota = `<h5><strong>Total a abonar por cuota:</strong> $${((carritoDeCompra.devolverValorFinal(catalogoPrecios)) / parseInt(carritoDeCompra.cuotasElegidas))}</h5>`
            let mensajeFinalParaConfirmacion = mensajeResumenPedido + mensajeValoresCarrito + mensajeSubTotal + cantidadCuotasElegidas + recargoCuotasElegidas + mensajeTotalAAbonar + mensajeTotalAAbonarPorCuota
            Swal.fire({
                title: 'Desea confirmar su compra?',
                icon: 'question',
                html: mensajeFinalParaConfirmacion,
                showCancelButton: true,
                confirmButtonColor: '#328B20',
                cancelButtonColor: '#d33',
